...
class TrtCudaGraphSafe {
   public:
    ...
    bool launch(cudaStream_t& stream) { return cudaGraphLaunch(mGraphExec, stream) == cudaSuccess; }
    ...
    void endCaptureOnError(cudaStream_t& stream) {
        ...
        cudaGetLastError();
        Trt::gLogError << "The CUDA graph capture on the stream has failed." << std::endl;
    }
