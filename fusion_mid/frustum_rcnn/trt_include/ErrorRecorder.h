...
//!
class TRTErrorRecorder : public IErrorRecorder {
    using errorPair = std::pair<ErrorCode, std::string>;
    using errorStack = std::vector<errorPair>;

   public:
    TRTErrorRecorder() = default;

    virtual ~TRTErrorRecorder() noexcept {}
    ...
    void clear() noexcept final {
        try {
            ...
            Trt::gLogFatal << "Internal Error: " << e.what() << std::endl;
        }
    };
    ...
    bool reportError(ErrorCode val, IErrorRecorder::ErrorDesc desc) noexcept final {
        try {
            std::lock_guard<std::mutex> guard(mStackLock);
            Trt::gLogError << "Error[" << static_cast<int32_t>(val) << "]: " << desc << std::endl;
