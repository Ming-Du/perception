...
nvinfer1::Dims toDims(std::vector<int32_t> const& vec) {
    ...
    if (static_cast<int32_t>(vec.size()) > limit) {
        Trt::gLogWarning << "Vector too long, only first 8 elements are used in dimension."
                         << std::endl;
    }
...
// bool broadcastIOFormats(std::vector<IOFormat> const& formats, size_t nbBindings, bool isInput /*=
// true*/)
// {
...
//                 "The number of inputIOFormats must match network's inputs or be one for
//                 broadcasting.");
//         }
//         else
//         {
//             throw std::invalid_argument(
//                 "The number of outputIOFormats must match network's outputs or be one for
//                 broadcasting.");
//         }
//     }
//     return broadcast;
...
void sparsifyMatMulKernelWeights(nvinfer1::INetworkDefinition& network,
    ...
    // Helper for analysis
    auto isTranspose = [](nvinfer1::Permutation const& perm) -> bool {
    ...
    for (auto& o2l : constO2L) {
        ...
        bool const all2D =
            is2D(mm->getInput(0)->getDimensions()) && is2D(mm->getInput(1)->getDimensions());
    ...
    auto sparsifyConstantWeights = [&sparseWeights](nvinfer1::IConstantLayer* layer,
...
template void setSparseWeights<IConvolutionLayer>(IConvolutionLayer& l,
                                                  int32_t k,
                                                  int32_t rs,
                                                  std::vector<int8_t>& sparseWeights);
template void setSparseWeights<IFullyConnectedLayer>(IFullyConnectedLayer& l,
                                                     int32_t k,
                                                     int32_t rs,
                                                     std::vector<int8_t>& sparseWeights);

void sparsify(nvinfer1::INetworkDefinition& network,
              std::vector<std::vector<int8_t>>& sparseWeights) {
...
void sparsify(Weights const& weights, int32_t k, int32_t rs, std::vector<int8_t>& sparseWeights) {
    switch (weights.type) {
        case DataType::kFLOAT:
            sparsify(static_cast<float const*>(weights.values), weights.count, k, rs,
                     sparseWeights);
            break;
        case DataType::kHALF:
            sparsify(static_cast<half_float::half const*>(weights.values), weights.count, k, rs,
                     sparseWeights);
            break;
        case DataType::kINT8:
...
// Explicit instantiation
template void dumpBuffer<bool>(void const* buffer,
                               std::string const& separator,
                               std::ostream& os,
                               Dims const& dims,
                               Dims const& strides,
                               int32_t vectorDim,
                               int32_t spv);
template void dumpBuffer<int32_t>(void const* buffer,
                                  std::string const& separator,
                                  std::ostream& os,
                                  Dims const& dims,
                                  Dims const& strides,
                                  int32_t vectorDim,
                                  int32_t spv);
template void dumpBuffer<int8_t>(void const* buffer,
                                 std::string const& separator,
                                 std::ostream& os,
                                 Dims const& dims,
                                 Dims const& strides,
                                 int32_t vectorDim,
                                 int32_t spv);
template void dumpBuffer<float>(void const* buffer,
                                std::string const& separator,
                                std::ostream& os,
                                Dims const& dims,
                                Dims const& strides,
                                int32_t vectorDim,
                                int32_t spv);
template void dumpBuffer<__half>(void const* buffer,
                                 std::string const& separator,
                                 std::ostream& os,
                                 Dims const& dims,
                                 Dims const& strides,
...
void sparsify(T const* values,
    ...
    auto const getIndex = [=](int32_t ki, int32_t ci, int32_t rsi) {
...
// Explicit instantiation
template void sparsify<float>(float const* values,
                              int64_t count,
                              int32_t k,
                              int32_t rs,
                              std::vector<int8_t>& sparseWeights);
template void sparsify<half_float::half>(half_float::half const* values,
                                         int64_t count,
                                         int32_t k,
                                         int32_t rs,
                                         std::vector<int8_t>& sparseWeights);
...
// Explicit instantiation
template void transpose2DWeights<float>(void* dst,
                                        void const* src,
                                        int32_t const m,
                                        int32_t const n);
template void transpose2DWeights<half_float::half>(void* dst,
                                                   void const* src,
                                                   int32_t const m,
                                                   int32_t const n);
