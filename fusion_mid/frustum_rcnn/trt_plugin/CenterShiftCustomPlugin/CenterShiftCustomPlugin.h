...
class CenterShiftCustomPlugin : public PluginDynamicBase {
 public:
  CenterShiftCustomPlugin(const std::string &name);

  CenterShiftCustomPlugin(const std::string name, const void *data,
                        size_t length);

  // It doesn't make sense to make TorchCumPluginDynamic without arguments, so
  // we delete default constructor.
  CenterShiftCustomPlugin() = delete;
...
class CenterShiftCustomPluginCreator : public PluginCreatorBase {
 public:
  CenterShiftCustomPluginCreator();
