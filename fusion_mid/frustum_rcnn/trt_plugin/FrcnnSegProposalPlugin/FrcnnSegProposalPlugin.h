...
class FrcnnSegProposalPlugin : public PluginDynamicBase {
 public:
  FrcnnSegProposalPlugin(const std::string &name, const int32_t point_num);

  FrcnnSegProposalPlugin(const std::string name, const void *data,
                        size_t length);

  // It doesn't make sense to make TorchCumPluginDynamic without arguments, so
  // we delete default constructor.
  FrcnnSegProposalPlugin() = delete;
  ...
  void setPluginNamespace(const char* np)   PLUGIN_NOEXCEPT;
...
class FrcnnSegProposalPluginCreator : public PluginCreatorBase {
 public:
  FrcnnSegProposalPluginCreator();
