...
#include "perception/fusion/common/environment.h"
#include "perception/fusion/common/file.h"
#include "common/include/log.h"
#include "perception/common/io/io_util.h"
...
namespace perception
{
  namespace lib
  {
    ...
    using perception::fusion::GetProtoFromASCIIFile;

    ConfigManager::ConfigManager()
    {
      work_root_ = FLAGS_work_root;

      ROS_DEBUG_STREAM("ConfigManager: FLAGS_work_root: " << FLAGS_work_root);

      // For start at arbitrary path
      if (work_root_.empty())
      {
        work_root_ = perception::fusion::GetEnv("MODULE_PATH");
        if (work_root_.empty())
        {
          work_root_ = perception::fusion::GetEnv("CYBER_PATH");
        }
      }
      ROS_DEBUG_STREAM("ConfigManager: FLAGS_work_root: " << FLAGS_work_root);
    }
    ...
    bool ConfigManager::InitInternal()
    {
      ...
      ROS_DEBUG_STREAM("InitInternal: WORK_ROOT: " << work_root_
                                                    << " config_root_path: "
                                                    << config_module_path);
      ...
      if (!common::GetFileList(config_module_path, "config_manager.config",
                               &model_config_files))
      {
        ROS_ERROR_STREAM("InitInternal: config_root_path : " << config_module_path
                                               << " get file list error.");
        return false;
      }
      ...
      {
        ROS_DEBUG_STREAM("InitInternal: model_config_file: " << model_config_file);
        ModelConfigFileListProto file_list_proto;
        if (!GetProtoFromASCIIFile(model_config_file, &file_list_proto))
        {
          ROS_ERROR_STREAM("InitInternal: Invalid ModelConfigFileListProto file: " << model_config_file);         
          return false;
        }
        ...
        {
          const std::string abs_path = work_root_+model_config_path;//GetAbsolutePath(work_root_, model_config_path);
          ROS_DEBUG_STREAM("InitInternal: abs_path: " << abs_path);
          MultiModelConfigProto multi_model_config_proto;
          if (!GetProtoFromASCIIFile(abs_path, &multi_model_config_proto))
          {           
            ROS_ERROR_STREAM("InitInternal: Invalid MultiModelConfigProto file: " << abs_path);
            return false;
          }
               ...
               multi_model_config_proto.model_configs())
          {
            ...
            ROS_DEBUG_STREAM("InitInternal: load ModelConfig succ. name: " << model_config->name());

            auto result =
                model_config_map_.emplace(model_config->name(), model_config);
            if (!result.second)
            {
              ROS_WARN_STREAM("InitInternal: duplicate ModelConfig, name: " << model_config->name());             
              return false;
            }
          }
        }
      ...
      ROS_DEBUG_STREAM("InitInternal: finish to load ModelConfigs. NumModels: "
    ...
    bool ModelConfig::Reset(const ModelConfigProto &proto)
    {
      ...
      ROS_DEBUG_STREAM("Reset: reset ModelConfig. model_name: " << name_
