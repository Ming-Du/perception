#include "perception/base/kalman_filter/kalman_filter.h"
#include "common/include/log.h"

namespace perception {
namespace common {

AdaptiveKalmanFilter::AdaptiveKalmanFilter() : init_(false), states_num_(0) {}

bool AdaptiveKalmanFilter::Init(const Eigen::VectorXd &initial_belief_states,
...
bool AdaptiveKalmanFilter::Predict(const Eigen::MatrixXd &transform_matrix,
...
bool AdaptiveKalmanFilter::Correct(const Eigen::VectorXd &cur_observation,
...
bool AdaptiveKalmanFilter::SetControlMatrix(const Eigen::MatrixXd &control_matrix) {
  if (!init_) {
...
Eigen::VectorXd AdaptiveKalmanFilter::GetStates() const { return global_states_; }

Eigen::MatrixXd AdaptiveKalmanFilter::GetUncertainty() const {
  return global_uncertainty_;
}

bool AdaptiveKalmanFilter::SetGainBreakdownThresh(const std::vector<bool> &break_down,
...
bool AdaptiveKalmanFilter::SetValueBreakdownThresh(const std::vector<bool> &break_down,
                                           const float threshold) {
  if (static_cast<int>(break_down.size()) != states_num_) {
    return false;
  }
...
void AdaptiveKalmanFilter::CorrectionBreakdown() {
...
bool AdaptiveKalmanFilter::DeCorrelation(int x, int y, int x_len, int y_len) {
