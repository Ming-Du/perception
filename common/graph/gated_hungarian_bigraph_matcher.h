...
#include "perception/common/graph/connected_component_analysis.h"
#include "perception/common/graph/hungarian_optimizer.h"
...
class GatedHungarianMatcher {
 public:
  /*@brief: option of optimization : max distance ; min distance. */
...
void GatedHungarianMatcher<T>::MatchInit() {
  ...
  if(!(find_ret != compare_fun_map.end())){
    std::cout << "MatchInit: MatchInit failed!" << std::endl;
    return;
  }
  ...
  if(is_valid_cost_(bound_value_)){
    std::cout << "MatchInit: is_valid_cost_, MatchInit failed! " << std::endl;
    return;
  }
}
