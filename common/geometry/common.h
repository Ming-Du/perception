...
#include "perception/fusion/base/box.h"
...
bool IsPointXYInPolygon2DXY(const PointT &point,
                            const perception::fusion::PointCloud<PointT> &polygon) {
...
template <typename Type>
Type CalculateIOUBBox(const perception::fusion::BBox2D<Type> &box1,
                      const perception::fusion::BBox2D<Type> &box2) {
  perception::fusion::Rect<Type> rect1(box1);
  perception::fusion::Rect<Type> rect2(box2);
  perception::fusion::Rect<Type> intersection = rect1 & rect2;
  perception::fusion::Rect<Type> unionsection = rect1 | rect2;
...
bool CalculateDistAndDirToSegs(
    ...
    const perception::fusion::PointCloud<PointT> &segs, typename PointT::Type *dist,
...
void CalculateDistAndDirToBoundary(
    const Eigen::Matrix<typename PointT::Type, 3, 1> &pt,
    const perception::fusion::PointCloud<PointT> &left_boundary,
    const perception::fusion::PointCloud<PointT> &right_boundary, typename PointT::Type *dist,
    Eigen::Matrix<typename PointT::Type, 3, 1> *dir) {
  using Type = typename PointT::Type;
...
void CalculateDistAndDirToBoundary(
    const Eigen::Matrix<typename PointT::Type, 3, 1> &pt,
    const common::EigenVector<perception::fusion::PointCloud<PointT>> &left_boundary,
    const common::EigenVector<perception::fusion::PointCloud<PointT>> &right_boundary,
