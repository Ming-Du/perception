...
bool SetProtoToASCIIFile(const google::protobuf::Message& message, int file_descriptor) {
  ...
  if (file_descriptor < 0) {
    ROS_ERROR("SetProtoToASCIIFile: Invalid file descriptor.");
    return false;
  }
...
bool SetProtoToASCIIFile(const google::protobuf::Message& message, const std::string& file_name) {
  ...
  if (fd < 0) {
    ROS_ERROR_STREAM("SetProtoToASCIIFile: Unable to open file " << file_name << " to write.");
    return false;
  }
...
bool GetProtoFromASCIIFile(const std::string& file_name, google::protobuf::Message* message) {
  ...
  if (file_descriptor < 0) {
    ROS_ERROR_STREAM("GetProtoFromASCIIFile: Failed to open file " << file_name
                                                                   << " in text mode.");
    // Failed to open;
    return false;
  }
  ...
  if (!success) {
    ROS_ERROR_STREAM("Parse: Failed to parse file " << file_name << " as text proto.");
  }
  delete input;
  close(file_descriptor);
  return success;
...
bool GetProtoFromBinaryFile(const std::string& file_name, google::protobuf::Message* message) {
  ...
  if (!input.good()) {
    ROS_ERROR_STREAM("GetProtoFromBinaryFile: Failed to open file " << file_name
                                                                    << " in binary mode.");
    return false;
  }
  if (!message->ParseFromIstream(&input)) {
    ROS_ERROR_STREAM("GetProtoFromBinaryFile: Failed to parse file " << file_name
                                                                     << " as binary proto.");
    return false;
  }
  return true;
...
bool RemoveAllFiles(const std::string& directory_path) {
  ...
  if (directory == nullptr) {
    ROS_ERROR_STREAM("RemoveAllFiles: Cannot open directory " << directory_path);
    return false;
  }
  ...
  while ((file = readdir(directory)) != nullptr) {
    ...
    if (unlink(file_path.c_str()) < 0) {
      ROS_ERROR_STREAM("RemoveAllFiles: Fail to remove file " << file_path << ": "
                                                              << strerror(errno));
      closedir(directory);
      return false;
    }
...
std::vector<std::string> ListSubPaths(const std::string& directory_path,
  ...
  DIR* directory = opendir(directory_path.c_str());
  if (directory == nullptr) {
    ROS_ERROR_STREAM("ListSubPaths: Cannot open directory " << directory_path);
    return result;
  }
...
bool GetType(const string& filename, FileType* type) {
  ...
  if (S_ISDIR(stat_buf.st_mode) != 0) {
    ...
    *type = TYPE_FILE;
    ...
    ROS_WARN_STREAM("GetType: failed to get type: " << filename);
    return false;
  }
  return true;
}
...
bool DeleteFile(const string& filename) {
  ...
  if (type == TYPE_FILE) {
    if (remove(filename.c_str()) != 0) {
      ROS_ERROR_STREAM("DeleteFile: failed to remove file: " << filename);
      return false;
    }
    return true;
  }
  ...
  if (dir == nullptr) {
    ROS_WARN_STREAM("DeleteFile: failed to opendir: " << filename);
    return false;
  }
  ...
  while ((dir_info = readdir(dir)) != nullptr) {
    ...
    if (!GetType(temp_file, &temp_type)) {
      ROS_WARN_STREAM("DeleteFile: failed to get file type: " << temp_file);
      closedir(dir);
      return false;
    }
    if (type == TYPE_DIR) {
...
bool CreateDir(const string& dir) {
  ...
  if (ret != 0) {
    ROS_WARN_STREAM(
        "CreateDir: failed to create dir. [dir: " << dir << "] [err: " << strerror(errno) << "]");
    return false;
  }
  return true;
}
