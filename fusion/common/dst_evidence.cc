...
bool DstManager::AddApp(const std::string& app_name,
  ...
  if (dst_common_data_.find(app_name) != dst_common_data_.end()) {
    ROS_WARN_STREAM(boost::format("AddApp: Dst %s was added!") % app_name);
  }
  ...
  if (dst_data.subsets_ind_map_.size() != dst_data.fod_subsets_.size()) {
    ROS_ERROR_STREAM(boost::format("AddApp: Dst %s: The input fod subsets"
...
DstCommonDataPtr DstManager::GetAppDataPtr(const std::string& app_name) {
  if (!IsAppAdded(app_name)) {
    ROS_ERROR("GetAppDataPtr: app_name is not available!");
    return nullptr;
  }
  auto iter = dst_common_data_.find(app_name);
  if (iter != dst_common_data_.end()) {
...
size_t DstManager::FodSubsetToInd(const std::string& app_name, const uint64_t& fod_subset) {
  ...
  if (!(iter0 != dst_common_data_.end())) {
    ROS_ERROR("FodSubsetToInd: iter0 == dst_common_data_.end().");
  }
  auto iter = iter0->second.subsets_ind_map_.find(fod_subset);
  if (!(iter != iter0->second.subsets_ind_map_.end())) {
    ROS_ERROR("FodSubsetToInd: iter == iter0->second.subsets_ind_map_.end().");
  }
  return iter->second;
}
...
uint64_t DstManager::IndToFodSubset(const std::string& app_name, const size_t& ind) {
  auto iter = dst_common_data_.find(app_name);
  if (!(iter != dst_common_data_.end())) {
    ROS_ERROR("IndToFodSubset: iter == dst_common_data_.end().");
  }
    ...
    for (size_t j = 0; j < dst_data->fod_subsets_.size(); ++j) {
      ...
      if (inter_res) {
        ...
        if (find_res == dst_data->subsets_ind_map_.end()) {
          ROS_ERROR_STREAM(
              boost::format("ComputeRelations: Dst: The input set "
...
void Dst::SelfCheck() const {
  if (!(DstManager::Instance()->IsAppAdded(app_name_))) {
    ROS_ERROR("SelfCheck::Instance()->IsAppAdded(app_name_) is failed.");
  }
...
bool Dst::SetBbaVec(const std::vector<double>& bba_vec) {
  ...
  if (bba_vec.size() != dst_data_ptr_->fod_subsets_.size()) {
    ROS_ERROR_STREAM(boost::format("SetBbaVec: input bba_vec size: %d !=  Dst subsets size: %d") %
  ...
  for (auto belief_mass : bba_vec) {
    if (belief_mass < 0.0) {
      ROS_WARN_STREAM(boost::format("SetBbaVec: belief mass: %lf is not valid") % belief_mass);
      return false;
    }
  }
  // reset
...
bool Dst::SetBba(const std::map<uint64_t, double>& bba_map) {
  ...
  for (auto bba_map_iter : bba_map) {
    ...
    if (find_res == subsets_ind_map.end()) {
      ROS_ERROR("SetBba: the input bba map has invalid fod subset!");
      return false;
    }
    if (belief_mass < 0.0) {
      ROS_WARN_STREAM(boost::format("SetBba: belief mass: %lf is not valid. Dst name: %s") %
...
std::string Dst::PrintBba() const {
  ...
  res += header + "\n";
  res += print_row("PrintBba: belief_mass", bba_vec_);
...
void Dst::ComputeSptPlsUct() const {
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    // ROS_DEBUG_STREAM(boost::format("inter_size: (%d %d)") % i % inter_inds.size());
    ...
    // ROS_DEBUG_STREAM(boost::format("pls: (%d %lf)") % i % pls);
    uct = pls - spt;
  }
}
...
void Dst::Normalize() {
  ...
  if (mass_sum == 0.0) {
    ROS_DEBUG("Normalize: mass_sum equal 0!!");
  }
...
Dst operator+(const Dst& lhs, const Dst& rhs) {
  CHECK_EQ(lhs.app_name_, rhs.app_name_)
      << boost::format("operator+: lhs Dst(%s) is not equal to rhs Dst(%s)") % lhs.app_name_ %
             rhs.app_name_;
  lhs.SelfCheck();
  rhs.SelfCheck();
  ...
  for (size_t i = 0; i < resbba_vec_.size(); ++i) {
    const auto& combination_pairs = combination_relations[i];
    // ROS_DEBUG_STREAM("pairs size: " << combination_pairs.size());
    double& belief_mass = resbba_vec_[i];
    belief_mass = 0.0;
    for (auto combination_pair : combination_pairs) {
      // ROS_DEBUG_STREAM(boost::format("(%d %d)") % combination_pair.first
      //     % combination_pair.second);
      belief_mass +=
          lhs.GetIndBfmass(combination_pair.first) * rhs.GetIndBfmass(combination_pair.second);
    }
    // ROS_DEBUG_STREAM(boost::format("belief_mass: %lf") % belief_mass);
  }
  res.Normalize();
  //处理一下resbba_vec_
  auto max_iter = std::max_element(resbba_vec_.begin(), resbba_vec_.end());
  if(*max_iter > 0.8){
    double prob_diff = (*max_iter - 0.8) / (resbba_vec_.size()-1);
    for (auto& belief_mass : resbba_vec_) {
      if(belief_mass != *max_iter){
        belief_mass += prob_diff;
      }else{
        belief_mass = 0.8;
      }
    }
...
Dst operator*(const Dst& dst, double w) {
  ...
  if (w < 0.0 || w > 1.0) {
    ROS_ERROR_STREAM(
        boost::format("operator*: the weight of bba %lf is not valid, return default bba") % w);
    return res;
  }
