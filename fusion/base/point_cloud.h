...
class AttributePointCloud : public PointCloud<PointT> {
 public:
  ...
  // @brief copy point from another point cloud
  inline bool CopyPoint(const size_t id,
                        const size_t rhs_id,
...
// Modify-guoxiaoxiao
inline PointFCloud ProtoArrayToPointCloud(const google::protobuf::RepeatedField<double>& points) {
  PointFCloud cloud_f;
  if (points.size() % 3 == 0) {
    ...
    ROS_WARN("ProtoArrayToPointCloud: point cloud data parsing not complete!!");
  }
  return cloud_f;
}
