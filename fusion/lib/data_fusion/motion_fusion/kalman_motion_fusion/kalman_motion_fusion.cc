...
bool KalmanMotionFusion::InitFilter(const SensorObjectConstPtr& sensor_object) {
  ...
  global_uncertainty.bottomRightCorner(2, 2) =
      sensor_object->GetBaseObject()
          ...
          .topLeftCorner(2, 2)  // origin:->velocity_uncertainty.topLeftCorner(2, 2)-syf
          .cast<double>();
...
void KalmanMotionFusion::UpdateWithoutMeasurement(const std::string& sensor_id,
  ...
  bool is_alive =
      (lidar_ptr != nullptr || radar_ptr != nullptr || camera_ptr != nullptr || obu_ptr != nullptr);
...
void KalmanMotionFusion::UpdateAnchorVelocity() {
  fused_anchor_point_(0) = kalman_filter_.GetStates()(0);
  fused_anchor_point_(1) = kalman_filter_.GetStates()(1);
  fused_velocity_(0) = kalman_filter_.GetStates()(2);
  fused_velocity_(1) = kalman_filter_.GetStates()(3);
}
void KalmanMotionFusion::UpdateVelocity() {
  fused_velocity_(0) = kalman_filter_.GetStates()(2);
  fused_velocity_(1) = kalman_filter_.GetStates()(3);
}
...
void KalmanMotionFusion::UpdateWithMeasurement(const SensorObjectConstPtr& measurement,
  ...
  if (is_lidar || is_radar || is_camera || is_obu || is_vidar) {
    if (filter_init_) {
  ...
  if (is_lidar || is_radar || is_obu || is_vidar) {
    ...
    // Modify(@liuxinyu): obu_test
    if (is_obu && lidar_ptr != nullptr) {
      // measurement is obu, has history lidar:
      // use fused achor point, use fused velocity
      //      fused_anchor_point_(0) = kalman_filter_.GetStates()(0);
      //      fused_anchor_point_(1) = kalman_filter_.GetStates()(1);
      //      fused_velocity_(0) = kalman_filter_.GetStates()(2);
...
void KalmanMotionFusion::MotionFusionWithMeasurement(const SensorObjectConstPtr& measurement,
  ...
  ROS_DEBUG_STREAM("MotionFusionWithMeasurement: fusion_original_measurement_covariance@("
                   << r_matrix(0, 0) << "," << r_matrix(0, 1) << "," << r_matrix(1, 0) << ","
  ...
  RewardRMatrixObu(measurement->GetSensorType(), measurement->GetBaseObject()->velocity_converged,
                   measurement->GetBaseObject()->source, &r_matrix);
  ...
  ROS_DEBUG_STREAM("MotionFusionWithMeasurement: fusion_filter_belief@("
                   << std::setprecision(10) << kalman_filter_.GetStates()(0) << ","
                   << kalman_filter_.GetStates()(1) << "," << kalman_filter_.GetStates()(2) << ","
                   << kalman_filter_.GetStates()(3) << ")");
  ROS_DEBUG_STREAM(
      "MotionFusionWithMeasurement: fusion_filter_belief_covariance@("
      << kalman_filter_.GetUncertainty()(0, 0) << "," << kalman_filter_.GetUncertainty()(0, 1)
      << "," << kalman_filter_.GetUncertainty()(1, 0) << ","
      << kalman_filter_.GetUncertainty()(1, 1) << "," << kalman_filter_.GetUncertainty()(2, 2)
      << "," << kalman_filter_.GetUncertainty()(2, 3) << ","
      << kalman_filter_.GetUncertainty()(3, 2) << "," << kalman_filter_.GetUncertainty()(3, 3)
      << ")");
}
...
void KalmanMotionFusion::RewardRMatrix(const base::SensorType& sensor_type,
  ...
  const float converged_scale_lp = 0.6f;  // modify by jiangnan
  const float converged_scale_lv_x = 0.75f;
  const float converged_scale_lv_y = 3.0f;

  const float converged_scale_c = 800.0f;

  const float converged_scale_rp = 2.0f;
  const float converged_scale_rv_x = 0.5f;
  const float converged_scale_rv_y = 2.5f;  // radar需要进一步验证
  // Modify(@liuxinyu): obu_test
  const float converged_scale_op = 0.001f;
  ...
  if (sensor_manager->IsLidar(sensor_type)) {
    if (converged) {
      ...
      r_matrix->block<1, 1>(2, 2) *= converged_scale_lv_x;
      r_matrix->block<1, 1>(3, 3) *= converged_scale_lv_y;
    } else {
      r_matrix->setIdentity();
      r_matrix->block<2, 2>(0, 0) *= converged_scale_lp;
    ...
    r_matrix->block<4, 4>(0, 0) *= converged_scale_c;
    ...
    if (lidar_history_length > 0) {
      r_matrix->block<4, 4>(0, 0).setIdentity();
    ...
    if (converged) {
      r_matrix->block<1, 1>(2, 2) *= converged_scale_rv_x;  // v_x  observation_noise
      r_matrix->block<1, 1>(3, 3) *= converged_scale_rv_y;
    } else {
      r_matrix->block<2, 2>(2, 2) *= unconverged_scale;
    }
  }
  // Modify(@liuxinyu): obu_test
  else if (sensor_manager->IsObu(sensor_type)) {
    r_matrix->setIdentity();
    r_matrix->block<6, 6>(0, 0).setIdentity();
    r_matrix->block<6, 6>(0, 0) *= converged_scale_op;
  }
  // r_matrix->block<2, 2>(4, 4) *= 0.5;
  r_matrix->block<2, 2>(4, 4) *= 5.0;  // Debug-guoxiaoxiao
}

void KalmanMotionFusion::RewardRMatrixObu(const base::SensorType& sensor_type,
                                          const bool& converged,
  ...
  const float converged_scale_rv_y = 2.5f;  // radar需要进一步验证
  // Modify(@liuxinyu): obu_test
  const float converged_scale_bsm_p = 0.001f;
  const float converged_scale_bsm_v = 0.001f;
  const float converged_scale_rsm_p = 0.1f;
  const float converged_scale_rsm_v = 0.1f;
  const float converged_scale_v2n_p = 0.1f;
  const float converged_scale_v2n_v = 0.1f;
  const float unconverged_scale = 1000.0f;
  ...
  // Modify(@liuxinyu): obu_test
    ...
    if (source == perception::fusion::ObjectSource::V2V_BSM) {
      r_matrix->block<2, 2>(0, 0) *= converged_scale_bsm_p;
      r_matrix->block<2, 2>(2, 2) *= converged_scale_bsm_v;
    } else if (source == perception::fusion::ObjectSource::V2I_RSM) {
      r_matrix->block<2, 2>(0, 0) *= converged_scale_rsm_p;
      r_matrix->block<2, 2>(2, 2) *= converged_scale_rsm_v;
    } else if (source == perception::fusion::ObjectSource::V2N_RSM) {
      r_matrix->block<2, 2>(0, 0) *= converged_scale_v2n_p;
      r_matrix->block<2, 2>(2, 2) *= converged_scale_v2n_v;
    } else {
      // other kind of
    }
  } else if (sensor_manager->IsVidar(sensor_type)) {  // ming.du
    r_matrix->setIdentity();
    r_matrix->block<2, 2>(0, 0) *= converged_scale_rp;
    if (converged) {
      r_matrix->block<1, 1>(2, 2) *= converged_scale_rv_x;  // v_x  observation_noise
...
Eigen::Vector4d KalmanMotionFusion::ComputePseudoMeasurement(const Eigen::Vector4d& measurement,
  ...
  // Modify(@liuxinyu): obu_test
  if (sensor_manager->IsObu(sensor_type)) {
    return ComputePseudoObuMeasurement(measurement);
  }
  ROS_DEBUG("ComputePseudoMeasurement: unsupport sensor type for pseudo measurement computation!");
  Eigen::Vector4d pseudo_measurement = measurement;
  return pseudo_measurement;
}

Eigen::Vector4d KalmanMotionFusion::ComputePseudoObuMeasurement(
    const Eigen::Vector4d& measurement) {
  // Modify(@liuxinyu): 复用的LIDAR
...
Eigen::Vector4d KalmanMotionFusion::ComputePseudoRadarMeasurement(
  ...
  // Modify(@liuxinyu): obu_test
  if (obu_ptr != nullptr) {
    obu_history_length = GetSensorHistoryLength(obu_ptr->GetSensorType());
  }
  // int lidar_camera_history_length = lidar_history_length + camera_history_length;
  int lidar_camera_obu_history_length =
      lidar_history_length + camera_history_length + obu_history_length;

  // Keep motion if lidar & camera & obu history is empty
  // if (lidar_camera_history_length == 0) {
  if (lidar_camera_obu_history_length == 0) {
  ...
  for (size_t count = 1; count < history_sensor_type_.size(); ++count) {
    ...
    // Modify(@liuxinyu): obu_test
    else if (sensor_manager->IsObu(history_type)) {
      obu_trace_count++;
      // Abandon lidar measurement, if its velocity angle change is too big
      if (fabs(velocity_angle_change) > M_PI / 20) {
        continue;
