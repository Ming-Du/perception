...
#include "common/extended_kalman_filter/extended_kalman_filter.h"
#include "lib/interface/base_motion_fusion.h"

namespace perception {
namespace fusion {

class EkfMotionFusion : public BaseMotionFusion {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

 public:
  explicit EkfMotionFusion(TrackPtr track) : BaseMotionFusion(track) {}
  ~EkfMotionFusion() = default;

  EkfMotionFusion(const EkfMotionFusion&) = delete;
  EkfMotionFusion& operator=(const EkfMotionFusion&) = delete;
  ...
  std::string Name() const override { return "EKFMotionFusion"; }
 ...
 private:
  ...
  ExtendedKalmanFilter ekf_filter_;
  ExtendedKalmanConfig ekf_filter_config_;
  Eigen::Vector3d fused_anchor_point_;
  Eigen::Vector3d fused_velocity_;
  Eigen::Vector3d fused_acceleration_;
  Eigen::VectorXd fused_state_;
  float fused_yaw_;
