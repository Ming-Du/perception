...
#include "common/proto/dst_existence_fusion_config.pb.h"
...
bool DstExistenceFusion::Init() {
  ...
  for (auto valid_dist : params.camera_valid_dist()) {
    ...
    ROS_DEBUG_STREAM("Init: dst existence fusion params: "
                     << camera_id
...
void DstExistenceFusion::UpdateWithMeasurement(const SensorObjectPtr measurement,
  ...
  fusion::ObjectConstPtr obj = measurement->GetBaseObject();
  ...
  ROS_DEBUG_STREAM(
      "UpdateWithMeasurement: before update exist prob: " << GetExistenceProbability());
  fused_existence_ = fused_existence_ + existence_evidence * exist_fused_w * association_prob;
  ROS_DEBUG_STREAM("UpdateWithMeasurement: update with, EXIST prob: "
                   << GetExistenceProbability() << " obj_id "
                   << measurement->GetBaseObject()->track_id << " association_prob: "
                   << association_prob << " decay: " << decay << " exist_prob: " << obj_exist_prob
...
void DstExistenceFusion::UpdateWithoutMeasurement(const std::string& sensor_id,
  ...
  if (perception::base::SensorManager::Instance()->IsCamera(sensor_id)) {
  ...
  SensorObjectConstPtr obu_object = track_ref_->GetLatestObuObject();
  SensorObjectConstPtr vidar_object = track_ref_->GetLatestVidarObject();
  if ((lidar_object != nullptr && lidar_object->GetSensorId() == sensor_id) ||
      (falcon_lidar_object != nullptr && falcon_lidar_object->GetSensorId() == sensor_id) ||
      (camera_object_latest != nullptr && camera_object_latest->GetSensorId() == sensor_id) ||
      (obu_object != nullptr && obu_object->GetSensorId() == sensor_id) ||
      (vidar_object != nullptr && vidar_object->GetSensorId() == sensor_id) ||
      (radar_object != nullptr && radar_object->GetSensorId() == sensor_id &&
       lidar_object == nullptr && camera_object_latest == nullptr && obu_object == nullptr &&
       falcon_lidar_object == nullptr)) {
    Dst existence_evidence(fused_existence_.Name());
    double unexist_factor = GetUnexistReliability(sensor_id);
    fusion::ObjectConstPtr obj = track_ref_->GetFusedObject()->GetBaseObject();
    ...
    // }
    ROS_DEBUG_STREAM("UpdateWithoutMeasurement: before update exist prob: "
                     << GetExistenceProbability() << " min_match_dist: " << min_match_dist
                     << " min_match_dist_score: " << min_match_dist_score);
    fused_existence_ =
        fused_existence_ + existence_evidence * unexist_fused_w * (1 - min_match_dist_score);
    ROS_DEBUG_STREAM("UpdateWithoutMeasurement: update without, EXIST prob: "
                     << GetExistenceProbability() << " 1 - match_dist_score: "
                     << 1 - min_match_dist_score << " sensor_id: " << sensor_id
                     << " dist_decay: " << dist_decay << " track_id: " << track_ref_->GetTrackId());
  }
  UpdateExistenceState();
}

double DstExistenceFusion::ComputeDistDecay(fusion::ObjectConstPtr obj,
  ...
  if (!status) {
    ROS_ERROR("ComputeDistDecay: Failed to get pose!");
    return dist_decay;
  }
  Eigen::Matrix4d world2sensor_pose = sensor2world_pose.matrix().inverse();
  if (!world2sensor_pose.allFinite()) {
    ROS_ERROR_STREAM(
        boost::format("ComputeDistDecay: The obtained camera pose is invalid. sensor_id : %s"
                      " timestamp %16.6f") %
        sensor_id % timestamp);
    return dist_decay;
  }
...
double DstExistenceFusion::ComputeFeatureInfluence(const SensorObjectPtr measurement) {
  ...
  ROS_DEBUG_STREAM("ComputeFeatureInfluence: sensor_id: "
                   << measurement->GetSensorId() << " velocity: " << velocity
...
double DstExistenceFusion::GetExistReliability(const SensorObjectPtr measurement) {
  bool unknown = (measurement->GetBaseObject()->type == fusion::ObjectType::UNKNOWN);
  ...
  if (sensor_manager->IsFalconLidar(measurement->GetSensorId())) {
    return 0.9 * unknown_ratio;
  }
  return 0.6;
}
...
double DstExistenceFusion::GetUnexistReliability(const std::string& sensor_id) {
  ...
  if (sensor_manager->IsFalconLidar(sensor_id)) {
    return 0.9;
  }
  if (sensor_manager->IsObu(sensor_id)) {
    return 0.9;
  }
  return 0.6;
}
...
void DstExistenceFusion::UpdateToicWithoutCameraMeasurement(const std::string& sensor_id,
  ...
  if (!(sensor_manager != nullptr)) {
    ROS_ERROR("UpdateToicWithoutCameraMeasurement: sensor_manager == nullptr.");
  }

  base::BaseCameraModelPtr camera_model = sensor_manager->GetCameraIntrinsic(sensor_id);
  if (!(camera_model != nullptr)) {
    ROS_ERROR_STREAM("UpdateToicWithoutCameraMeasurement: Failed to get camera intrinsic for "
                     << sensor_id);
  }
  ...
  perception::base::SensorManager* sensor_manager2 = perception::base::SensorManager::Instance();
  if (!sensor_manager2->GetSensorInfo(sensor_id, &camera_sensor_info_))
    ROS_ERROR_STREAM(
        "UpdateToicWithoutCameraMeasurement: camera get sensor info failure : " << sensor_id);
  auto type = fusion::kMogoSensorType2NameMap.find(camera_sensor_info_.type());
  if (type == fusion::kMogoSensorType2NameMap.end()) {
    ROS_WARN("UpdateToicWithoutCameraMeasurement: No name map from Mogo SensorType.");
  }
  ...
  if (max_dist_it == options_.camera_max_valid_dist_.end()) {
    ROS_WARN_STREAM(
        boost::format("UpdateToicWithoutCameraMeasurement: There is no pre-defined max valid camera"
                      " dist for sensor type: %s")
        << sensor_id);
  }
  if (status && max_dist_it != options_.camera_max_valid_dist_.end() &&
      type != fusion::kMogoSensorType2NameMap.end()) {
    SensorObjectConstPtr lidar_object = track_ref_->GetLatestLidarObject();
    SensorObjectConstPtr radar_object = track_ref_->GetLatestRadarObject();
...
void DstExistenceFusion::UpdateToicWithCameraMeasurement(const SensorObjectPtr& camera_obj,
  ...
  base::BaseCameraModelPtr camera_model = sensor_manager->GetCameraIntrinsic(sensor_id);
  if (!(camera_model != nullptr)) {
    ROS_ERROR_STREAM("UpdateToicWithCameraMeasurement: Failed to get camera intrinsic for "
                     << sensor_id);
  }
  ...
  if (!sensor_manager2->GetSensorInfo(sensor_id, &camera_sensor_info_))
    ROS_ERROR_STREAM(
        "UpdateToicWithCameraMeasurement: camera get sensor info failure : " << sensor_id);
  auto type = fusion::kMogoSensorType2NameMap.find(camera_sensor_info_.type());
  if (type == fusion::kMogoSensorType2NameMap.end()) {
    ROS_WARN("UpdateToicWithCameraMeasurement: No name map from  Mogo SensorType");
  }
  auto max_dist_it = options_.camera_max_valid_dist_.find(type->second);
  // auto max_dist_it = options_.camera_max_valid_dist_.find(sensor_id);      //org
  if (max_dist_it == options_.camera_max_valid_dist_.end()) {
    ROS_WARN_STREAM(
        boost::format("UpdateToicWithCameraMeasurement: There is no pre-defined max valid camera"
                      " dist for sensor type: %s") %
        sensor_id);
  } else {
    ROS_DEBUG_STREAM("UpdateToicWithCameraMeasurement: camera dist: " << sensor_id << " "
                                                                      << max_dist_it->second);
  }
  if (status && max_dist_it != options_.camera_max_valid_dist_.end() &&
      type != fusion::kMogoSensorType2NameMap.end()) {
...
void DstExistenceFusion::UpdateExistenceState() {
  ...
  } else if (!track_ref_->GetObuObjects().empty()) {
    toic_score_ = 0.5;
  } else if (!track_ref_->GetVidarObjects().empty()) {
    toic_score_ = 0.5;  // ming.du question why 0.5
  } else if (!(track_ref_->GetFalconLidarObjects().empty())) {
    toic_score_ = 0.5;  // Modify @jiangnan: add falcon state
  } else {
    ROS_ERROR_STREAM(
        boost::format(
            "UpdateToicWithCameraMeasurement: this fused object: %d has no sensor objects") %
