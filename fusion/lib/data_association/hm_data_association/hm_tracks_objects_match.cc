...
#include "perception/common/graph/secure_matrix.h"
...
double HMTrackersObjectsAssociation::s_association_center_dist_threshold_ =
    30.0;  // origin 30, modified to 300 in 2021-06-01, only camera and radar,
           // for camera object, center is calculated by intrinsic parameter
           // -syf
...
bool HMTrackersObjectsAssociation::Associate(const AssociationOptions& options,
  ...
  // get foregroud objects and tracks for association
  ...
  // either of objects and tracks is empty, set both unassigned.
  ...
  bool do_nothing = sensor_manager->IsRadar(measurement_sensor_id) || sensor_manager->IsVidar(measurement_sensor_id);
  IdAssign(fusion_tracks, sensor_objects, &association_result->assignments,
           ...
           do_nothing, false);
  // ROS_DEBUG_STREAM("IdAssigned tracks num =" << association_result->assignments.size());//
  // Modify-guoxiaoxiao

  Eigen::Affine3d pose;
  sensor_measurements->GetPose(&pose);
  Eigen::Vector3d ref_point = pose.translation();
  //  ROS_DEBUG_STREAM("association_measurement_timestamp@" <<
  //  measurement_timestamp);
  bool is_camera = sensor_manager->IsCamera(measurement_sensor_id);
  bool is_radar = sensor_manager->IsRadar(measurement_sensor_id);

  ComputeAssociationDistanceMat(
      fusion_tracks, sensor_objects, ref_point, association_result->unassigned_tracks,
      association_result->unassigned_measurements, &association_mat, is_camera);
  ...
  ROS_DEBUG_STREAM("association: measurement_num = "
                   << sensor_objects.size() << ", track_num = " << fusion_tracks.size()
...
void HMTrackersObjectsAssociation::PostIdAssign(
  ...
  auto is_valid_track = [](const TrackPtr& fusion_track) {
    ...
    return camera_obj != nullptr && fusion_track->GetLatestLidarObject() == nullptr &&
           fusion_track->GetLatestFalconLidarObject() == nullptr;
  };
...
void HMTrackersObjectsAssociation::ComputeDistance(
    ...
    const std::vector<std::vector<double>>& association_mat,
    AssociationResult* association_result) {
  // track2measurements_dist store distances of association_mat after IdAssign.
  ...
  for (size_t i = 0; i < association_result->unassigned_tracks.size(); i++) {
    ...
    SensorObjectConstPtr falcon_lidar_object =
        fusion_track->GetLatestFalconLidarObject();  // Modify @jiangnan
...
void HMTrackersObjectsAssociation::ComputeAssociationDistanceMat(
  ...
  for (size_t i = 0; i < unassigned_tracks.size(); ++i) {
    ...
    //@lijian,如果是camera测量值，在此处计算一次track的投影点，存储在Object中，内部计算使用
    // ConvertTracktoimages(fusion_track);
    if (!fusion_track->GetFusedObject()->GetBaseObject()->centers_image.empty() && iscamera) {
      fusion_track->GetFusedObject()->GetBaseObject()->centers_image.clear();
    }
    for (size_t j = 0; j < unassigned_measurements.size(); ++j) {
      ...
      double center_dist = (sensor_object->GetBaseObject()->position -
                            fusion_track->GetFusedObject()->GetBaseObject()->position)
                               .norm();

      if (center_dist < s_association_center_dist_threshold_ || iscamera == true) {
        distance = track_object_distance_.Compute(fusion_track, sensor_object, opt);
      }
      (*association_mat)[i][j] = distance;
      // ROS_DEBUG_STREAM("ComputeAssociationDistanceMat: track_id: " <<
      // fusion_track->GetTrackId()
      //        << ", obs_id: " << sensor_object->GetBaseObject()->track_id
      //        << ", distance: " << distance);
    }
  }
  if (iscamera) {
    const std::vector<int> samematchindex = SelectMinMatchDistanceforTrack((*association_mat));
    AdjustAssociationMat(fusion_tracks, unassigned_tracks, samematchindex, association_mat);
  }
}
std::vector<int> HMTrackersObjectsAssociation::SelectMinMatchDistanceforTrack(
    std::vector<std::vector<double>>& association_mat) {
  std::vector<int> minDistanceSensorObjIndex;
  //矩阵合理性检查和异常判断：代价矩阵为空返回空数组
  if (association_mat.empty())
    return minDistanceSensorObjIndex;
  if (association_mat[0].empty()) {
    return minDistanceSensorObjIndex;
  }
  //根据track的数量分配数组的尺寸
  minDistanceSensorObjIndex.resize(association_mat.size());
  //存储下最小距离代价的测量目标的所在位置
  for (int m = 0; m < association_mat.size(); ++m) {
    auto mindistance = std::min_element(association_mat[m].begin(), association_mat[m].end());
    int index = std::distance(std::begin(association_mat[m]), mindistance);
    minDistanceSensorObjIndex[m] = index;
  }
  return minDistanceSensorObjIndex;
}
void HMTrackersObjectsAssociation::AdjustAssociationMat(
    const std::vector<TrackPtr>& fusion_tracks,
    const std::vector<size_t>& unassigned_tracks,
    const std::vector<int>& mindistanceindex,
    std::vector<std::vector<double>>* association_mat) {
  //输入数据安全性检查,低于两个元素没有必要修改
  if (mindistanceindex.size() < 2)
    return;
  //两遍遍历数据找到重复匹配测量值的track[可优化]
  for (int m = 0; m < (mindistanceindex.size() - 1); ++m) {
    if ((*association_mat)[m][mindistanceindex[m]] >= 4.0)
      continue;
    for (int n = m + 1; n < mindistanceindex.size(); ++n) {
      if ((*association_mat)[n][mindistanceindex[n]] >= 4.0)
        continue;
      if (mindistanceindex[m] == mindistanceindex[n]) {
        //判断一下哪个track离自车更近
        const TrackPtr t1 = fusion_tracks[unassigned_tracks[m]];
        const TrackPtr t2 = fusion_tracks[unassigned_tracks[n]];
        double d1 = t1->GetFusedObject()->GetBaseObject()->center[0];
        double d2 = t2->GetFusedObject()->GetBaseObject()->center[0];
        if (d1 >= d2) {  
          (*association_mat)[m][mindistanceindex[m]] = 4.0;
        } else {
          (*association_mat)[n][mindistanceindex[n]] = 4.0;
        }
      }
    }
  }
...
void HMTrackersObjectsAssociation::IdAssign(const std::vector<TrackPtr>& fusion_tracks,
  ...
  for (size_t i = 0; i < num_obj; i++) {
    ...
    if (!post && (sensor_manager->IsCamera(sensor_id)))
      continue;
    if (it != sensor_Id_2_track_ind.end()) {
      double delta_px = fabs(sensor_objects[i]->GetBaseObject()->position[0] -
                             fusion_tracks[it->second]->GetFusedObject()->GetBaseObject()->position[0]);
      double delta_py = fabs(sensor_objects[i]->GetBaseObject()->position[1] -
                             fusion_tracks[it->second]->GetFusedObject()->GetBaseObject()->position[1]);

      if (delta_px < idassign_diff_px && delta_py < idassign_diff_py) {
        sensor_used[i] = true;
