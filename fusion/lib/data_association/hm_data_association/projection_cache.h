...
#include "perception/common/include/math_utils.h"

#include "base/sensor_object.h"
#define POINTS_SIZE 600000
...
class ProjectionCacheObject {
 public:
  ProjectionCacheObject() : start_ind_(0), end_ind_(0) { box_ = fusion::BBox2DF(); }
...
class ProjectionCacheFrame {
 public:
  ...
  bool VerifyKey(std::string sensor_id, double timestamp) {
    return sensor_id_ == sensor_id && ::common::math::almost_equal(timestamp_, timestamp, 2);
  }
...
class ProjectionCache {
 public:
  ProjectionCache() : measurement_sensor_id_(""), measurement_timestamp_(0.0) {
    ...
    // velodyne64
    // point2ds_.reserve(300000);     //syf-not enough for HESAI_128
    point2ds_.reserve(POINTS_SIZE);
  }
 ...
 private:
  bool VerifyKey(const std::string& sensor_id, double timestamp) {
    return measurement_sensor_id_ == sensor_id &&
           ::common::math::almost_equal(measurement_timestamp_, timestamp, 2);
  }
